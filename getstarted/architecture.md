...
七牛大数据平台Pandora的产品架构图如上图所示。数据入口是多样化的，用户可以通过使用我们的采集客户端直接收集数据发送到大数据处理平台，也可以自主通过Pandora平台提供的Rest接口输入数据，还可以通过Pandora提供的针对不同语言的SDK将数据发送到大数据处理平台中。进入大数据处理平台以后，数据可以经过一系列的数据清洗、变换、聚合，最终根据需要导出到下游其他服务。
...
### 产品关系说明
1. 在公有云中，`大数据处理平台`是Pandora产品集的数据唯一入口；
