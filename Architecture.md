...
### 数据闭环
1. 在`时序数据库`以及`日志检索服务`中的数据是有存储时限的，最长30天；也就是说任何一条数据在`时序数据库`/`日志检索服务`当中不会超过30天。如果希望永久储存数据，建议将数据导出一份至`对象存储服务`或`HTTP地址`/`Mongo DB`。
2. `消息队列`中的数据是隐式的，也就是说我们无法查询堆积在`消息队列`中的具体数据；并且`消息队列中`的数据目前只会存储2天时间。
3. `计算任务`是消耗物理资源的，当用户购买的资源较少，同时数据量输入过大时，就会产生严重的数据堆积；这种堆积可能会造成数据丢失。